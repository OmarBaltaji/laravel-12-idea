  ...
  <div class="py-8 max-w-4xl mx-auto">
    <div class="flex justify-between">
      ...
      <div class="flex items-center gap-x-3">
        <button x-data class="btn btn-outlined" data-test="edit-idea-button" idea="{{ $idea }}" @click="$dispatch('open-modal', 'edit-idea')">
          <x-icons.external />
          Edit Idea
        </button>

        <form method="POST" action="{{ route('ideas.delete', ['idea' => $idea]) }}">
          @csrf
          @method("DELETE")
        ...
        <div>
          ...
          <div class="mt-3 space-y-2">
            @foreach ($idea->links as $link)
              <x-card target="_blank" rel="noopener noreferrer" :href="$link" class="text-primary font-medium flex gap-x-3 items-center">
                <x-icons.external />
                {{ $link }}
              </x-card>
            @endforeach
